...
} = cartAdapter.getSelectors<RootState>((state: RootState) => 
  state.cart
);
...
export const selectCartTotal = createSelector(
  selectCartItems,
  (items: CartItem[]) => items.reduce((total: number, item: CartItem) => total + item.price * item.qty, 0)
);

// Toplam ürün adedi
export const selectCartCount = createSelector(
  selectCartItems,
  (items: CartItem[]) => items.reduce((count: number, item: CartItem) => count + item.qty, 0)
);
...
export const selectIsInCart = (productId: number) => createSelector(
  selectCartItems,
  (items: CartItem[]) => items.some((item: CartItem) => item.id === productId)
); 